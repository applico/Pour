...
router.get('/KegName', function(req, res) {
  res.send({KegName : kegName});
});

router.post('/KegName/new/:newBeerName', function(req, res) {
  res.send("Changed Beer Name to " + req.newBeerName);
});
...
router.param('newBeerName', function(req, res, next, newBeerName) {
  kegName = newBeerName;
  req.newBeerName = newBeerName;
  next();
});
...
router.post('/kegHealth/add/:addKegHealthValue', function(req, res) {
  res.send("Added Value to KegHealth: " + Number(req.addKegHealthValue) + "%");
});

router.post('/kegHealth/del/:delKegHealthValue', function(req, res) {
  res.send("Deleted Value from KegHealth: " + Number(req.delKegHealthValue) + "%");
});
...
router.param('addKegHealthValue', function(req, res, next, addKegHealthValue) {
  KegHealth += Number(addKegHealthValue);
  req.addKegHealthValue = addKegHealthValue;
  next();
});

router.param('delKegHealthValue', function(req, res, next, delKegHealthValue) {
  KegHealth -= Number(delKegHealthValue);
  req.delKegHealthValue = delKegHealthValue;
  next();
});

router.get('/health', function(req, res) {
  res.send([{Keg : KegHealth},{CO2Level : CO2},{name : kegName}]);
});
    ...
    if (!err) {
      try {
        ...
        if (inUser[0].cid == req.body.cid) {
          ...
          console.log("replacing old user, still posting pour");
          

          try {
            console.log("hi")
            mongoose.Users.findOneAndRemove({cid : inUser[0].cid}, function(err, redundancy) {
              console.log("deleted" + redundancy);
            });
          }catch (err) {
            console.log("error at user doc removal");
          }
          ...
          return res.send("user cid is redundant, replacing");
        }
      } catch (err) {
        console.log(err);
